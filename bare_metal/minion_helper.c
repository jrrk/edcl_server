...
struct bootstrap_host { int flags; const struct sdhci_ops *ops; unsigned long _private[0]; };
struct mmc_request;
...
uint32_t queue_block_read2(uint32_t *buf2, int i)
{
  uint32_t rslt = __be32_to_cpu(buf2[i]);
  return rslt;
}
...
static void minion_sdhci_read_block_pio(uint32_t *tmp)
{
	unsigned long flags;
	...
	while (blksize) {
	  ...
	  while (len) {
	    if (chunk == 0) {
              scratch = queue_block_read2(tmp, i++);
	      chunk = 4;
	    }
...
static void minion_sdhci_write_block_pio(void)
{
	unsigned long flags;
	size_t blksize, len, chunk;
	u32 scratch;	
      ...
      if ((card_status[8] < card_status[25]) && card_status[9])
	{
	  if (card_status[20] & 0x10)
	    {
		{
		  enum {max=512};
		  uint32_t tmp[max];
		  int cnt = queue_block_read1(tmp, max);
		  if (cnt != 129)
		    printf("transf_cnt = %d, fifo_cnt = %d\n", card_status[9], cnt);
		  minion_sdhci_read_block_pio(tmp);
		}
	    }
	}
      else
